...
static void applyBrightness() { StickCP2.Display.setBrightness(50 + brightLevel * 50); }
...
void menuConfirm() {
  switch (menuSel) {
    ...
    case 1: esp_deep_sleep_start(); break;
    case 2:
    case 3:
      menuOpen = false;
...
static void clockRefreshRtc() {
  ...
  _clkLastRead = millis();
  _onUsb = StickCP2.Power.isCharging();
  StickCP2.Rtc.getTime(&_clkTm);
  StickCP2.Rtc.getDate(&_clkDt);
}
...
static void drawClock() {
  ...
  StickCP2.Display.setRotation(clockOrient);
  static uint8_t lastSec = 0xFF;
  bool repaint = paintedOrient != clockOrient;
  if (repaint) { StickCP2.Display.fillScreen(p.bg); paintedOrient = clockOrient; lastSec = 0xFF; }
  ...
  if (repaint || _clkTm.seconds != lastSec) {
    ...
    StickCP2.Display.setTextSize(3); StickCP2.Display.setTextColor(p.text, p.bg);    StickCP2.Display.drawString(hm, 170, 42);
    StickCP2.Display.setTextSize(2); StickCP2.Display.setTextColor(p.textDim, p.bg); StickCP2.Display.drawString(ssl, 170, 72);
                                                                  StickCP2.Display.drawString(wdl, 170, 102);
  ...
  if (millis() - lastPetTick >= 200) {
    ...
    if (buddyMode) {
      ...
      StickCP2.Display.fillRect(0, 0, 115, 90, p.bg);
      buddyRenderTo(&StickCP2.Display, activeState);
    } else {
      ...
      characterRenderTo(&StickCP2.Display, 57, 45);
    }
  }
...
void drawInfo() {
  ...
  if (infoPage == 0) {
    ...
    int vBat_mV = StickCP2.Power.getBatteryVoltage();
    int iBat_mA = (int)StickCP2.Power.getBatteryCurrent();
    bool usb = StickCP2.Power.isCharging();
